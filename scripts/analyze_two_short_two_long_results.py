"""
Load 2s/2l sweep pickle files, rank strategies, and plot top performers.
...
Three plots are written to tmp/two_short_two_long_sweeps/plots overlaying
...
class LoadedResult:
    ...
    def label(self) -> str:
        exp = self.params.get("expiry_range", (None, None))
        hold = self.params.get("hold_period")
        s1 = self.params.get("short1_delta")
        s2 = self.params.get("short2_delta")
        l1 = self.params.get("long1_delta")
        l2 = self.params.get("long2_delta")
        lr1 = self.params.get("long1_ratio")
        lr2 = self.params.get("long2_ratio")
        return (
            f"s({s1:.3f},{s2:.3f}) "
            f"l({lr1}x{l1:.3f},{lr2}x{l2:.3f}) "
            f"exp {exp[0]}-{exp[1]} hold {hold}"
        )
