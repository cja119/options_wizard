...
class Universe:
    ...
    def top_constituents(self, n_const: int) -> None:
        ...
        index_df = pd.read_csv(
            os.getenv("INDEX_CONSTITUENTS_PATH"),
            na_values=na_vals
        )
        mktcap_df = pd.read_csv(
            os.getenv("MKT_CAP_PATH"),
            na_values=na_vals
        )
        ...
        top_n_per_date = panel_clean.apply(lambda row: row.nlargest(n_const).index.tolist(), axis=1)
    ...
    def check_ticks(self):
        ...
        for stock in self.ticks.copy():
            if stock not in available_ticks:
                print(f"\r Removing {stock} from universe - data not available", end=" "*10)
